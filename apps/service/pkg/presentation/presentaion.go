...
func NewServer(mux *http.ServeMux, db *gorm.DB) {
	...
	api.Handle(odachinconnect.NewAuthServiceHandler(server, intercepters))
	api.Handle(odachinconnect.NewAllowanceServiceHandler(server, intercepters))
	api.Handle(odachinconnect.NewFamilyServiceHandler(server, intercepters))
	api.Handle(odachinconnect.NewRewardServiceHandler(server, intercepters))
	api.Handle(odachinconnect.NewTransactionServiceHandler(server, intercepters))
	api.Handle(odachinconnect.NewUsageServiceHandler(server, intercepters))
